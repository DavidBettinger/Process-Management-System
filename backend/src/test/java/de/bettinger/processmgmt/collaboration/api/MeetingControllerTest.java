...
class MeetingControllerTest {
	...
	@Test
	void updatesScheduledMeeting() throws Exception {
		String tenantId = "tenant-1";
		UUID caseId = UUID.randomUUID();
		UUID locationId = seedLocation(tenantId, "Kita Sonnenblume");
		MeetingEntity scheduled = meetingCommandService.scheduleMeeting(
				tenantId,
				caseId,
				locationId,
				"Kickoff",
		...
		mockMvc.perform(put("/api/cases/{caseId}/meetings/{meetingId}", caseId, scheduled.getId())
						.header(DevAuthFilter.USER_HEADER, "u-1")
						.header(DevAuthFilter.TENANT_HEADER, tenantId)
						.contentType(MediaType.APPLICATION_JSON)
						.content(payload))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.id").value(scheduled.getId().toString()))
				.andExpect(jsonPath("$.status").value("SCHEDULED"))
				.andExpect(jsonPath("$.locationId").value(locationId.toString()))
				.andExpect(jsonPath("$.participantIds[0]").value("u-1"))
				.andExpect(jsonPath("$.participantIds[1]").value("u-2"))
				.andExpect(jsonPath("$.title").value("Kickoff aktualisiert"))
				.andExpect(jsonPath("$.description").value("Neue Agenda"))
				.andExpect(jsonPath("$.scheduledAt").value("2026-02-03T09:30:00Z"));
	}

	@Test
	void rejectsInvalidMeetingUpdatePayload() throws Exception {
		String tenantId = "tenant-1";
		UUID caseId = UUID.randomUUID();
		UUID locationId = seedLocation(tenantId, "Kita Sonnenblume");
		MeetingEntity scheduled = meetingCommandService.scheduleMeeting(
	...
	@Test
	void returnsNotFoundForUnknownMeetingUpdate() throws Exception {
		String tenantId = "tenant-1";
		UUID caseId = UUID.randomUUID();
		UUID locationId = seedLocation(tenantId, "Kita Sonnenblume");
		...
		mockMvc.perform(put("/api/cases/{caseId}/meetings/{meetingId}", caseId, meetingId)
						.header(DevAuthFilter.USER_HEADER, "u-1")
						.header(DevAuthFilter.TENANT_HEADER, tenantId)
						.contentType(MediaType.APPLICATION_JSON)
						.content(payload))
